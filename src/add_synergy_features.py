...
ALPHA_SYNERGY = 10.0  # smoothing para pairs
ALPHA_VS      = 10.0
...
def compute_team_synergy(team_ids, synergy_games, synergy_wins, alpha):
    # suma de winrates de todos los pares del equipo
    vals = []
...
def compute_vs_advantage(blue_ids, red_ids, vs_games, vs_wins, alpha):
    # Blue_vs_Red: suma de p_hat(i,j) para i en Blue, j en Red
    vals_blue = []
    vals_red = []
    for i in blue_ids:
        for j in red_ids:
            # ventaja de i sobre j
            g_ij = vs_games[i, j]
            w_ij = vs_wins[i, j]
            p_ij = smoothed_rate(w_ij, g_ij, alpha)
            vals_blue.append(p_ij)
            # ventaja de j sobre i (simétrica)
            g_ji = vs_games[j, i]
            w_ji = vs_wins[j, i]
...
def main():
    ...
    print(f"✅ Guardado {MATCH_FEATS_OUT} con sinergias y VS.")
